...
load('..\..\..\..\ArticalCode\MyMvda\SLCG\SLCGfeatures\DMvDAvc_SLCG_UCMerced.mat');
DimsNum=length(DMvDAvc);
Accuracy=zeros(RcnN,DimsNum);
Dims=cell(1,DimsNum);

for i=1:DimsNum
    ClassDataC=[];
%     globalfeatureall=DR_HistNorm(DMvDAvc{i});
    globalfeatureall=DMvDAvc{i};
...
% 
Multi_MvDAvc_UCMerced_PR.DimsNum=Dims;
Multi_MvDAvc_UCMerced_PR.MethodName='MvDAvc';
Multi_MvDAvc_UCMerced_PR.Accuracy=Accuracy;
Multi_MvDAvc_UCMerced_PR.R_train_all=Features.R_train_all;
Multi_MvDAvc_UCMerced_PR.R_test_all=Features.R_test_all;
Multi_MvDAvc_UCMerced_PR.meanV=mean(Accuracy);
Multi_MvDAvc_UCMerced_PR.maxV=max(Accuracy);
save('.\ClassResults\Md_MvDAvc_UCMerced_Classifier_PR.mat','Multi_MvDAvc_UCMerced_PR');
% 
% 
% % % 
