...
load('..\..\..\MSE(MultiviewSpectralEmbbeding)\Features\DMSE_SLCG_AID.mat');

DimsNum=length(AID_MSE.feature);
Accuracy=zeros(RcnN,DimsNum);
Dims=cell(1,DimsNum);

for i=1:DimsNum
    ClassDataC=[];
    globalfeatureall=DR_HistNorm(AID_MSE.feature{i});
...
% 
Multi_MSE_AID_PR.DimsNum=Dims;
Multi_MSE_AID_PR.MethodName='MSE';
Multi_MSE_AID_PR.Accuracy=Accuracy;
Multi_MSE_AID_PR.R_train_all=Features.R_train_all;
Multi_MSE_AID_PR.R_test_all=Features.R_test_all;
Multi_MSE_AID_PR.meanV=mean(Accuracy);
Multi_MSE_AID_PR.maxV=max(Accuracy);
save('.\ClassResults\Md_MSE_AID_Classifier_PR.mat','Multi_MSE_AID_PR');
% 
% 
% % % 
